...
use aoc_2025_02::{count_invalid_ids, is_invalid2};
use std::fs;
...
fn solve(data: &str) -> String {
    data.split(",")
        ...
        .map(|(begin, end)| count_invalid_ids(begin, end, &is_invalid2))
        .sum::<i64>()
        .to_string()
}

fn main() {
    let filecontent = fs::read_to_string("input.txt")
        .expect("Failed to read input file");
    
    let result = solve(&filecontent);
    println!("{}", result);
    
    fs::write("result2.txt", &result)
